...
public class ShapingTimer implements Runnable {
    ...
    public void run() {
        ...
        if (count == 2) shaper.configure(0, TWO_GB);
        if (count == 3) shaper.configure(0, FOUR_GB);
        if (count == 4) shaper.configure(0, SIX_GB);
        if (count == 5) shaper.configure(0, EIGHT_GB);
        if (count == 5) shaper.configure(0, TEN_GB);
